...
def dokmeans(np_vecs,in_file,k_clusters):
    ...
    k=len(k_clusters)
    init_vecs = []
    for cluster_el in k_clusters:
        init_vecs.append( np.mean( cluster_el  ,axis=0 ) )
    init_vecs = np.array( init_vecs  )
    print(init_vecs)
    #init_vecs = np.array( k_clusters )
    #init_vecs = np.mean( k_clusters  ,axis=0 )
    km              = KMeans(n_clusters=k,n_init=k,max_iter=10000,init=init_vecs)#init='kmeans++'
    kmf             = km.fit(np_vecs)
    dict_cnt        = {}
    membership_dict = {}
